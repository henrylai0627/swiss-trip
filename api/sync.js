...
async function geminiMerge(baselineDoc, newDoc, days) {
  ...
  // responseSchema = 約束 Gemini 嘅輸出結構(constrained decoding),令佢冇得吐壞 JSON
      ...
      patches: {
        ...
        items: {
          ...
          properties: {
            ...
            title: { type: "string" },
            route: { type: "string" },
            hotel: { type: "object", properties: { nm: { type: "string" }, loc: { type: "string" } } },
            costs: { type: "array", items: { type: "string" } },
            tips: { type: "array", items: { type: "string" } },
            meals: { type: "array", items: { type: "object", properties: { b: { type: "string" }, v: { type: "string" } } } },
          },
          required: ["id"],
        },
      },
  ...
  if (s < 0 || e < 0) throw new Error("Gemini 冇回傳 JSON: " + txt.slice(0, 200));
  ...
  try {
    ...
    // 防禦:清走字串內裸露嘅控制字元再試
    try {
      return JSON.parse(slice.replace(/[\x00-\x1F]+/g, " "));
    } catch (_) {
      throw new Error(`${err.message} | Gemini 原始輸出: ${slice.slice(0, 400)}`);
    }
  }
}
